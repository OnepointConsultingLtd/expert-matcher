    ...
    return (
        <div className="mt-0">
            <div className="flex flex-row justify-left mb-2">
                <input type="search" value={suggestionFilter} onChange={(e) => setSuggestionFilter(e.target.value)} 
                    placeholder={t('Search suggestions')}
                    className='border-2 border-gray-300 rounded-md p-2 w-full max-w-1/2'></input>
            </div>
            ...
            <div className="container suggestions animate-fade-down grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 max-w-full">
                {suggestions.filter(s => s.length === 0 || s.toLowerCase().includes(suggestionFilter.toLowerCase())).map((suggestion, i) => {
